  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-background border shadow-lg rounded-lg p-3 text-sm">
        <p className="font-bold text-lg text-foreground">{payload[0].value.toLocaleString()} sales</p>
        <p className="text-muted-foreground">${(payload[0].value * 0.78).toLocaleString()}</p>
      </div>
    );
  }
  return null;
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription className="flex items-center gap-2">
              ...
              <span className="text-xs text-green-500 font-medium">+1.3% VS LAST YEAR</span>
            </CardDescription>
          </div>
          <div className="flex items-center gap-1 rounded-md bg-muted p-1">
            <Button variant="ghost" size="sm" className="h-7 text-xs">Daily</Button>
            <Button variant="ghost" size="sm" className="h-7 text-xs">Weekly</Button>
            <Button variant="secondary" size="sm" className="h-7 text-xs">Annually</Button>
          </div>
        </div>
      </CardHeader>
